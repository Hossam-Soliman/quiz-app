...
const renderAnswerStatus = (value) => {
	const { correct_answer, points, possible_answers } = questionsData.questions[currentTab],
	...
	if (Array.isArray(correct_answer)) {
		...
		correct_answer.forEach((outerEl, i) => {
			...
			correctAnswerString += `${answer.caption} ${i !== correct_answer.length - 1 ? '& ' : ''}`;
		});
	} else if (typeof correct_answer === 'number') {
		const parsedValue = parseInt(value);
		isCorrect = parsedValue === correct_answer;

		correctAnswerString = possible_answers.find((el) => el.a_id === correct_answer).caption;
...
const validateForm = () => {
	...
	if (valid) {
		...
		} else if (questionsData.questions[currentTab].question_type === 'multiplechoice-multiple') {
			currentInputValue = $(`#${questionsData.questions[currentTab].q_id}`).val();
		}
...
const showTab = (n) => {
	...
	if (questionsData.questions[currentTab].question_type === 'multiplechoice-multiple') {
...
const fetchData = async () => {
	try {
		...
		const questionsApi = await fetch('https://proto.io/en/jobs/candidate-exercise/quiz.json'),
			resultsApi = await fetch('https://proto.io/en/jobs/candidate-exercise/result.json');
		...
		form.innerHTML += '';
		form.innerHTML += '<h3>Something went wrong while fetching data, please try again later</h3>';
	}
};
